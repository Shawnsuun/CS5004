...
public interface GenericListADTNode<T> {
  ...
  <R> GenericListADTNode<R> map(Function<T,R> converter);
    ...
    /**
   * A general filter higher order function on nodes. This returns a list of
   * identical structure, but filtered based on the sent predicate
   * @param predicate the filter test
   * @return the head of a list that is structurally identical to this list,
   * but contains only items that pass the predicate
   */
